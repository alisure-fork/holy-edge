...
class Vgg16(object):
    ...
    def __init__(self, config, run='training'):
        self.config = config
        self.tool = Tool()

        self.data_dict = np.load(self.config['model_weights_path'], encoding='latin1').item()
        self.tool.print_info("Model weights loaded from {}".format(self.config['model_weights_path']))

        self.images = tf.placeholder(tf.float32, [None, self.config[run]['image_height'],
                                                  self.config[run]['image_width'], self.config[run]['n_channels']])
        self.edge_maps = tf.placeholder(tf.float32,
                                        [None, self.config[run]['image_height'], self.config[run]['image_width'], 1])
    ...
    def define_model(self):
        ...
        self.tool.print_info('Added CONV-BLOCK-1+SIDE-1')
        ...
        self.tool.print_info('Added CONV-BLOCK-2+SIDE-2')
        ...
        self.tool.print_info('Added CONV-BLOCK-3+SIDE-3')
        ...
        self.tool.print_info('Added CONV-BLOCK-4+SIDE-4')
        ...
        self.tool.print_info('Added CONV-BLOCK-5+SIDE-5')

        self.side_outputs = [self.side_1, self.side_2, self.side_3, self.side_4, self.side_5]

        self.fuse = self.conv_layer(tf.concat(self.side_outputs, axis=3), [1, 1, len(self.side_outputs), 1],
                                    name='fuse_1', use_bias=False, w_init=tf.constant_initializer(0.2))

        self.tool.print_info('Added FUSE layer')

        # complete output maps from side layer and fuse layers
        self.outputs = self.side_outputs + [self.fuse]

        self.data_dict = None
        self.tool.print_info("Build model finished: {:.4f}s".format(time.time() - start_time))
        pass
    ...
    def conv_layer(self, x, w_shape, b_shape=None, name=None, padding='SAME', use_bias=True, w_init=None, b_init=None):
    ...
    def setup_training(self, session):
        ...
        self.predictions = []
        self.loss = 0
        self.tool.print_warning('Deep supervision application set to {}'.format(self.config['deep_supervision']))

        for idx, b in enumerate(self.side_outputs):
            output = tf.nn.sigmoid(b, name='output_{}'.format(idx))
            cost = sigmoid_cross_entropy_balanced(b, self.edge_maps, name='cross_entropy{}'.format(idx))

            self.predictions.append(output)
            if self.config['deep_supervision']:
                self.loss += (self.config['loss_weights'] * cost)
                pass
            pass

        fuse_output = tf.nn.sigmoid(self.fuse, name='fuse')
        fuse_cost = sigmoid_cross_entropy_balanced(self.fuse, self.edge_maps, name='cross_entropy_fuse')

        self.predictions.append(fuse_output)
        self.loss += (self.config['loss_weights'] * fuse_cost)

        pred = tf.cast(tf.greater(fuse_output, 0.5), tf.int32, name='predictions')
        error = tf.cast(tf.not_equal(pred, tf.cast(self.edge_maps, tf.int32)), tf.float32)
        ...
        self.train_writer = tf.summary.FileWriter(self.config['train_dir'] + '/train', session.graph)
        self.val_writer = tf.summary.FileWriter(self.config['train_dir'] + '/val')
        pass
